...
WITH Revenues AS
(
	SELECT sc.CompanyName + ' (' + sc.FirstName + ' ' + sc.LastName + ')' AS CompanyContract,
		   SUM(h.TotalDue) AS Revenue
	FROM SalesLT.Customer sc JOIN SalesLT.SalesOrderHeader h ON sc.CustomerID = h.CustomerID
